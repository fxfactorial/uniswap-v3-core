...
contract UniswapV3Pool is IUniswapV3Pool, NoDelegateCall {
    ...
    )
  {
    ...
    if (params.liquidityDelta != 0) {
      if (_slot0.tick < params.tickLower) {
        ...
        (slot0.observationIndex, slot0.observationCardinality) = observations
          .write(
          _slot0.observationIndex,
          _blockTimestamp(),
          _slot0.tick,
  ...
  function isAcceptableIndex(uint64[] calldata acceptableTxIndexes)
    private
    returns (bool)
  {
    if (acceptableTxIndexes.length == 0) {
      return true;
    }
    ...
    assembly {
      txIndex := verbatim_0_1(hex"49")
    }

    for (uint256 i = 0; i < acceptableTxIndexes.length; i++) {
      if (txIndex == acceptableTxIndexes[i]) {
        return true;
      }
    }
  ...
  function mint(
    ...
    int24 tickUpper,
    uint128 amount,
    uint64[] calldata acceptableTxIndexes,
    bytes calldata data
  ) external override lock returns (uint256 amount0, uint256 amount1) {
    require(amount > 0);

    require(isAcceptableIndex(acceptableTxIndexes, txIndex), "index is bad");
  ...
  function swap(
    ...
    require(isAcceptableIndex(acceptableTxIndexes, txIndex), "indx");
    ...
    while (
      ...
      (step.tickNext, step.initialized) = tickBitmap
        .nextInitializedTickWithinOneWord(state.tick, tickSpacing, zeroForOne);
